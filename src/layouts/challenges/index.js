...
// Data
import brandsNameTable from "layouts/challenges/data/brandsNameTable";
...
function Brands() {
  ...
  const fetchAllBrandsCategories = async () => {
    ...
    const predefinedCategories = [
      { id: 'plasticWaste', name: 'Plastic Waste' },
      { id: 'paperWaste', name: 'Paper Waste' },
      { id: 'energyConservation', name: 'Energy Conservation' },
      { id: 'greenTransportation', name: 'Green Transportation' },
      { id: 'waterConservation', name: 'Water Conservation' },
      // Add more categories as needed
    ];    
  
    setCategoriesDropdown(predefinedCategories);
  };
  ...
  const onAddBrand = async (e) => {
    ...
    const defaultParticipants = Math.floor(100 + Math.random()).toString();
    const customDocId = Math.floor(10000000 + Math.random() * 90000000).toString();
  
    // Post data into firestore
    try {
      setLoading(true);
      await setDoc(doc(db, "brands", customDocId), {
        ...
        participants: defaultParticipants, // Set default value for Participants
        uid: customDocId,
      });
    ...
    <>
      ...
      >
        <BootstrapDialogTitle id="customized-dialog-title" onClose={brandsModalClose}>
          <Typography variant="h3" color="secondary.main" sx={{ pt: 1, textAlign: "center" }}>Add Challenge</Typography>
          ...
          >
            ...
            <Box sx={{ maxWidth: "100%", m: 2 }}>
              <FormControl fullWidth >
                <InputLabel id="demo-simple-select-label" sx={{ height: "2.8rem" }} required>Select Challenge Category</InputLabel>
                <Select
                  sx={{ height: "2.8rem" }}
